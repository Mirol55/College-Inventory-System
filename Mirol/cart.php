...
<body>
      ...
      ?>
         <form action="" method="POST" class="box">
            ...
            <div class="flex">
               ...
               <input type="number" name="qty" class="qty" max="99" min="1" maxlength="2" required
                  value="<?= $fetch_cart['qty']; ?>">
               <button type="submit" name="update_qty" class="fas fa-edit"></button>
            </div>
            <p class="sub-total"><span>Sub total :</span>
               <?= $sub_total = ($fetch_cart['qty'] * $fetch_product['price']); ?></p>
            <input type="submit" value="remove from cart" name="remove_item" class="delete-btn"
   ...
   <section>
      ...
      <form action="" class="count-container" method="POST">
         ...
         <input type="submit" value="delete all" name="delete_all" onclick="return confirm('delete all from cart?');"
            class="inline-delete-btn <?= ($grand_total > 1)?'':'disabled'; ?>">
      </form>

      <div class="flex-btn" style="margin-top: 30px;">
         <a href="newmenu.php" class="inline-option-btn">continue shopping</a>
         <a href="payment.php" class="inline-btn <?= ($grand_total > 1)?'':'disabled'; ?>">proceed to checkout</a>
      </div>
   ...
   <script>
      document.querySelectorAll('input[type="number"]').forEach(inputNumbmer => {
         inputNumbmer.oninput = () => {
            if (inputNumbmer.value.length > inputNumbmer.maxLength) inputNumbmer.value = inputNumbmer.value
               .slice(0, inputNumbmer.maxLength);
         }
      });
   </script>
